/**
 * 🏮 EL FAROL AL DÍA — V34.3
 * Cambios vs V34.2:
 *  - Watermark: búsqueda flexible con 8 variantes de nombre, fallo silencioso
 *  - Gemini: modelo cambiado a gemini-2.0-flash (estable), timeout 60s con AbortController
 *  - Railway: generación secuencial en RSS (no paralela), retry con backoff
 *  - AdSense CPC: términos inyectados por categoría de forma natural en el prompt
 *  - Limpieza de bucles de error y manejo de excepciones mejorado
 */
...
// ─── WATERMARK: búsqueda flexible, fallo silencioso ─────────────────────────
// FIX V34.3: agrega todas las variantes posibles de nombre incluyendo WATERMARK(1).png
...
const WATERMARK_PATH = (() => {
    for (const name of WATERMARK_CANDIDATES) {
        const fullPath = path.join(__dirname, 'static', name);
        if (fs.existsSync(fullPath)) {
            console.log('Watermark encontrado: ' + name);
            return fullPath;
        }
    }
    console.warn('AVISO: No se encontró watermark.png en /static. Se publicará sin marca de agua.');
    return null; // null = sin watermark, no rompe el proceso
})();
...
async function publicarEnTwitter(titulo, slug, descripcion) {
    ...
    try {
        ...
        console.log('   Twitter OK ID: ' + data.data?.id);
        return true;
...
async function bienvenidaTelegram() {
    ...
    try {
        await fetch(`https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ chat_id: id, text: `*El Farol al Dia - Bot Activo*\n\nConectado y listo.\n\n[elfarolaldia.com](https://elfarolaldia.com)`, parse_mode: 'Markdown' }),
        });
    } catch (e) {}
}

// ─── WATERMARK ───────────────────────────────────────────────────────────────
// FIX V34.3: Si WATERMARK_PATH es null, retorna la URL original sin procesar
async function aplicarMarcaDeAgua(urlImagen) {
    // Sin watermark configurado: publicar sin marca, no romper el flujo
    if (!WATERMARK_PATH) {
        console.log('   Sin watermark configurado, usando imagen original');
    ...
    try {
        ...
        const bufFin  = await sharp(bufOrig)
            .composite([{ input: wmRes, left: Math.max(0, w - wmAncho - margen), top: Math.max(0, h - wmAlto - margen), blend: 'over' }])
            .jpeg({ quality: 88 })
            .toBuffer();
        ...
        console.warn('Watermark error (continuando sin marca): ' + err.message);
        return { url: urlImagen, procesada: false };
    }
}
...
async function cargarConfigIA() {
    try {
        ...
        if (r.rows.length) {
            ...
            console.log('Config IA desde BD');
...
// ─── GEMINI — FIX V34.3 ──────────────────────────────────────────────────────
// Modelo cambiado a gemini-2.0-flash (estable en v1beta)
// Timeout 60s con AbortController para no colgar Railway
// Retry con backoff exponencial, máximo 3 intentos
const GS = { lastRequest: 0, resetTime: 0 };
const GEMINI_MODEL   = 'gemini-2.0-flash';
const GEMINI_TIMEOUT = 60000; // 60 segundos

async function llamarGemini(prompt, reintentos = 3) {
    for (let i = 0; i < reintentos; i++) {
        try {
            console.log(`   Gemini intento ${i + 1}/${reintentos} (${GEMINI_MODEL})`);
            ...
            const tm   = setTimeout(() => ctrl.abort(), GEMINI_TIMEOUT);
            ...
            if (res.status === 429) {
                ...
                console.warn(`   Rate limit 429, esperando ${espera}ms`);
                await new Promise(r => setTimeout(r, espera));
                continue;
            }
            if (res.status === 503 || res.status === 502) {
                const espera = Math.pow(2, i) * 4000;
                console.warn(`   Gemini ${res.status}, reintento en ${espera}ms`);
                await new Promise(r => setTimeout(r, espera));
                continue;
            }
            ...
            console.error(`   Gemini ERROR intento ${i + 1}: ${esTimeout ? 'TIMEOUT (60s)' : err.message}`);
            if (i < reintentos - 1) {
                const espera = Math.pow(2, i) * 3000;
                await new Promise(r => setTimeout(r, espera));
            }
        }
    }
...
async function obtenerImagenInteligente(titulo, categoria, subtema, queryIA) {
    ...
    const u2 = await buscarEnPexels([`${categoria} dominican republic`]);
    if (u2) return u2;
...
async function inicializarBase() {
    ...
    try {
        ...
        console.error('BD init error: ' + e.message);
    } finally {
        client.release();
    }
...
async function regenerarWatermarksLostidos() {
    if (!WATERMARK_PATH) { console.log('Sin watermark: omitiendo regeneracion'); return; }
    try {
...
// ─── ADSENSE CPC ALTO — términos por categoría ───────────────────────────────
// FIX V34.3: Tabla más amplia y descripción de cómo integrarlos naturalmente
const ADSENSE_CPC = {
...
async function generarNoticia(categoria, comunicadoExterno = null) {
    try {
        ...
        // Terminos AdSense CPC alto — se inyectan como instruccion de contexto
...
- Parrafo 4 IMPACTO ECONOMICO: Que cambia para el ciudadano. Menciona NATURALMENTE, sin forzar, el impacto en aspectos como: ${termCPC}
...
- QUERY_IMAGEN: 3-4 palabras en ingles, escena periodistica (NO bodas mascotas moda)
...
CONTENIDO:
[400-500 palabras, 5 parrafos separados por linea en blanco]`;

        console.log('\nGenerando: ' + categoria + (comunicadoExterno ? ' (RSS/Comunicado)' : ' (Auto)'));
        ...
        // Publicar en redes sociales en paralelo (no bloquea la respuesta)
        Promise.allSettled([
            ...
            const [fb, tw, tg] = results.map((r, i) => {
                const labels = ['FB', 'TW', 'TG'];
                return r.status === 'fulfilled' && r.value ? `${labels[i]} OK` : `${labels[i]} ERR`;
            });
            console.log('   Redes: ' + [fb, tw, tg].join(' | '));
        }).catch(e => console.warn('Redes error: ' + e.message));

        return { success: true, slug: slFin, titulo, mensaje: 'Publicada en web + redes' };

    } catch (error) {
        console.error('ERROR generarNoticia: ' + error.message);
        await registrarError('generacion', error.message, categoria);
        return { success: false, error: error.message };
    }
}

// ─── RSS — 30 FUENTES, PROCESAMIENTO SECUENCIAL ───────────────────────────────
// FIX V34.3: Procesamiento SECUENCIAL (no paralelo) para no saturar Railway ni Gemini
const FUENTES_RSS = [
...
// Bandera para evitar ejecuciones paralelas del procesador RSS
let rssEnProceso = false;

async function procesarRSS() {
    if (!CONFIG_IA.enabled) return;
    if (rssEnProceso) { console.log('RSS: ya en proceso, omitiendo'); return; }
    rssEnProceso = true;
    console.log('\nRSS: iniciando ciclo (' + FUENTES_RSS.length + ' fuentes)');
    let procesadas = 0;

    // SECUENCIAL: una fuente a la vez para no saturar Railway
        ...
        try {
            ...
            for (const item of feed.items.slice(0, 3)) {
                ...
                if (res.success) {
                    ...
                    procesadas++;
                    // Pausa entre noticias para no saturar Gemini
                    await new Promise(r => setTimeout(r, 6000));
                }
                break; // Solo 1 item por fuente por ciclo
            }
        } catch (err) {
    ...
    console.log('RSS: ciclo completo, ' + procesadas + ' noticias nuevas');
    rssEnProceso = false;
}
...
// Keep-alive Railway (cada 14 min)
cron.schedule('*/14 * * * *', async () => {
    try { await fetch(`http://localhost:${PORT}/health`); } catch (e) {}
});

// Generar 1 noticia aleatoria cada 4 horas
cron.schedule('0 */4 * * *', async () => {
    if (!CONFIG_IA.enabled) return;
    await generarNoticia(CATS[Math.floor(Math.random() * CATS.length)]);
});

// Procesar RSS 4 veces al día (secuencial, no paralelo)
...
// ─── RUTAS ───────────────────────────────────────────────────────────────────
app.get('/health', (req, res) => res.json({ status: 'OK', version: '34.3', modelo: GEMINI_MODEL }));
...
app.post('/api/procesar-rss', authMiddleware, async (req, res) => {
    ...
    procesarRSS(); // no awaiteamos — respuesta inmediata
...
app.get('/api/configuracion', (req, res) => {
    try {
        ...
        const c = fs.existsSync(cfgPath) ? JSON.parse(fs.readFileSync(cfgPath, 'utf8')) : { googleAnalytics: '' };
...
app.get('/status', async (req, res) => {
    try {
        ...
        res.json({
            status: 'OK', version: '34.3', modelo_gemini: GEMINI_MODEL,
            ...
            marca_de_agua:      WATERMARK_PATH ? 'Activa (' + path.basename(WATERMARK_PATH) + ')' : 'No configurada (publicando sin marca)',
            ia_activa:          CONFIG_IA.enabled,
            rss_en_proceso:     rssEnProceso,
            sistema:            'Web + FB + TW + TG + RSS 30 fuentes + Wikipedia + Watermark flexible + SEO + AdSense CPC',
        });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
async function iniciar() {
    ...
    app.listen(PORT, '0.0.0.0', () => {
        console.log(`
╔══════════════════════════════════════════════════════╗
║         🏮 EL FAROL AL DIA — V34.3                  ║
║  Web + Facebook + Twitter + Telegram + Wikipedia     ║
║  RSS 30 fuentes (secuencial) + SEO + AdSense CPC     ║
╠══════════════════════════════════════════════════════╣
║  Modelo Gemini : ${GEMINI_MODEL.padEnd(34)}║
║  Timeout IA    : 60s con AbortController             ║
║  Facebook      : ${(FB_PAGE_ID && FB_PAGE_TOKEN ? 'ACTIVO' : 'Sin credenciales').padEnd(34)}║
║  Twitter       : ${(TWITTER_API_KEY && TWITTER_ACCESS_TOKEN ? 'ACTIVO' : 'Sin credenciales').padEnd(34)}║
║  Telegram      : ${(TELEGRAM_TOKEN ? 'ACTIVO' : 'Sin token').padEnd(34)}║
║  Watermark     : ${(WATERMARK_PATH ? path.basename(WATERMARK_PATH) : 'No encontrado — publicando sin marca').padEnd(34)}║
╚══════════════════════════════════════════════════════╝`);
    });
