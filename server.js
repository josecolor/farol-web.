 ...
 * Búnker PRO v2.0 - VERSIÓN DEFINITIVA (REVISADA)
 * - Usa MONGO_URL (variable de Railway) sin fallback
 * - Fuerza IPv4 con family: 4 (evita error :1:27017)
...
// ==================== VALIDACIÓN DE MONGO_URL (variable de Railway) ====================
const MONGO_URL = process.env.MONGO_URL;

if (!MONGO_URL) {
    console.error('\n❌ ERROR CRÍTICO: La variable MONGO_URL no está definida en Railway.');
    console.error('👉 Ve a la pestaña Variables de tu proyecto y créala con el valor de conexión.');
    process.exit(1); // Detiene el proceso, Railway lo reiniciará hasta que la variable exista
}

console.log('📡 MONGO_URL encontrada. Conectando a MongoDB...');

// ==================== CONEXIÓN A MONGODB CON REINTENTOS BLOQUEANTES ====================
    ...
    for (let i = 1; i <= intentos; i++) {
        try {
            ...
            await mongoose.connect(MONGO_URL, {
                ...
                family: 4,                     // Fuerza IPv4 (clave para evitar el error :1:27017)
                maxPoolSize: 10,
                minPoolSize: 2,
                maxIdleTimeMS: 30000,
            ...
            if (i === intentos) {
                console.error('\n🔴 No se pudo conectar a MongoDB después de varios intentos. Saliendo...');
                process.exit(1); // Termina el proceso, Railway lo reiniciará
            }
...
const noticiaSchema = new mongoose.Schema({
    ...
    redactor: { type: String, default: 'mxl', trim: true },
...
// ==================== FUNCIÓN PARA INYECTAR METAETIQUETAS SSR ====================
async function inyectarMetaTags(html) {
    try {
        ...
        if (config && config.googleVerification) {
            ...
            // Limpiar por si el usuario pegó la etiqueta completa
            ...
            console.log('ℹ️ No hay código de verificación, se eliminó el marcador.');
        }
        return html;
    } catch (error) {
...
// ==================== MIDDLEWARE PARA API 404 ====================
// Si una ruta que empieza con /api no se encuentra, devolvemos JSON
app.use('/api/*', (req, res) => {
    res.status(404).json({ success: false, error: 'API endpoint no encontrado' });
});

// ==================== MANEJO DE ERRORES PARA RUTAS NO API (REDIRECCIÓN A PORTADA) ====================
app.use((req, res) => {
...
async function iniciarServidor() {
    await conectarMongoDB(); // Esta función es bloqueante: si falla, sale del proceso
