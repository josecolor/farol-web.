...
// ══════════════════════════════════════════════════════════
// 🌐 CORS ABIERTO — Blogger y cualquier sitio puede leer /api/noticias
// ══════════════════════════════════════════════════════════
app.use(cors({
    origin: '*',
...
app.options('*', cors()); // Responder preflight de todos los orígenes
...
// ── KEEP-ALIVE — ping cada 5 minutos para no dormir ──
cron.schedule('*/5 * * * *', async () => {
    try { await fetch(`http://localhost:${PORT}/health`); } catch(e) {}
});

// ── MODO NORMAL — una noticia cada 30 minutos ──
cron.schedule('*/30 * * * *', async () => {
    if (!CONFIG_IA.enabled) return;
    // Solo corre si ya pasó el período de ráfaga inicial (35 min desde arranque)
...
// ── RÁFAGA INICIAL — 4 noticias al arrancar, cada 2 minutos ──
// Después de 30 min de descanso, el cron normal toma el control
...
async function rafagaInicial() {
    ...
    // Noticia 1 — inmediata con imagen Pexels
    try {
        const cat1 = CATS[Math.floor(Math.random() * CATS.length)];
        console.log(`📰 Ráfaga 1/4: ${cat1}`);
        await generarNoticia(cat1);
    } catch(e) { console.error('Ráfaga 1 error:', e.message); }

    // Noticia 2 — 2 minutos
    await new Promise(r => setTimeout(r, 2 * 60 * 1000));
    try {
        const cat2 = CATS[Math.floor(Math.random() * CATS.length)];
        console.log(`📰 Ráfaga 2/4: ${cat2}`);
        await generarNoticia(cat2);
    } catch(e) { console.error('Ráfaga 2 error:', e.message); }

    // Noticia 3 — 4 minutos desde arranque
    await new Promise(r => setTimeout(r, 2 * 60 * 1000));
    try {
        const cat3 = CATS[Math.floor(Math.random() * CATS.length)];
        console.log(`📰 Ráfaga 3/4: ${cat3}`);
        await generarNoticia(cat3);
    } catch(e) { console.error('Ráfaga 3 error:', e.message); }

    // Noticia 4 — 6 minutos desde arranque
    await new Promise(r => setTimeout(r, 2 * 60 * 1000));
    try {
        const cat4 = CATS[Math.floor(Math.random() * CATS.length)];
        console.log(`📰 Ráfaga 4/4: ${cat4}`);
        await generarNoticia(cat4);
    } catch(e) { console.error('Ráfaga 4 error:', e.message); }
...
// ── TELEGRAM — endpoint de prueba y status ──
app.get('/api/telegram/status', authMiddleware, async (req, res) => {
    ...
    res.json({
        ...
        instruccion:   chatId
            ...
            : '⚠️ Escríbele cualquier mensaje al bot en Telegram para activarlo'
    });
});
...
app.post('/api/telegram/test', authMiddleware, async (req, res) => {
    ...
    const ok = await publicarEnTelegram(
        ...
        'El bot de El Farol al Día está activo. ¡Listo para Último Minuto en Santo Domingo Este y toda RD!',
        'Nacionales'
    );
    res.json({
        success: ok,
        mensaje: ok ? '✅ Mensaje enviado a Telegram' : '❌ Error — escríbele al bot primero'
    });
});
...
// Manejar preflight OPTIONS explícitamente para Blogger
...
app.get('/api/noticias', async (req, res) => {
    // Headers CORS explícitos — Blogger y cualquier sitio puede leer esto
    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('Access-Control-Allow-Methods', 'GET, OPTIONS');
    res.setHeader('Access-Control-Allow-Headers', 'Content-Type');
...
╚══════════════════════════════════════════════════════════════════╝`);
    ...
    // 15 segundos después: ráfaga inicial de 4 noticias cada 2 min
