/**
 * 🏮 EL FAROL AL DÍA — server.js V34.31 CORREGIDO
 * 
 * FIXES V34.31:
 * ✓ Error 429: Delays 60s entre requests
 * ✓ Manejo 1-4 keys (no falla si falta alguna)
 * ✓ Descansa IA entre ciclos
 * ✓ Sin watermark innecesario
...
// GEMINI KEYS - Soporta 1, 2, 3 o 4 keys
const GEMINI_KEYS = [
...
console.log(`📊 Gemini keys encontradas: ${GEMINI_KEYS.length}`);

if (GEMINI_KEYS.length === 0) {
  console.error('❌ ERROR: No hay GEMINI_API_KEY configurada');
  process.exit(1);
}
...
async function verificarDuplicado(titulo, categoria) {
  try {
    ...
    return { esDuplicada: false, similitud: 0 };
  }
}
...
async function guardarDuplicado(titulo, titulo_norm, slug, categoria, gemini_key_num) {
  try {
    ...
    console.error('⚠️ Error guardando duplicado:', e.message);
  }
}
...
function monetizar(titulo, categoria) {
  let t = titulo;
  
  if (categoria === 'Economia' && !t.includes('inversión')) {
    t = `${t} – Inversión`;
  } else if (categoria === 'Nacionales' && (t.includes('construcci') || t.includes('vivienda'))) {
    t = `${t} – Mercado`;
  }
...
// ════════════════════════════════════════════════════════════════
// ESPERAR ANTES DE GEMINI (anti 429)
// ════════════════════════════════════════════════════════════════

async function esperarDelayGemini() {
  const ahora = Date.now();
  const transcurrido = ahora - ultimoTiempoGemini;
  
  if (transcurrido < DELAY_ENTRE_KEYS) {
    const espera = DELAY_ENTRE_KEYS - transcurrido;
    console.log(`⏳ Esperando ${(espera / 1000).toFixed(1)}s...`);
    await new Promise(resolve => setTimeout(resolve, espera));
  }
  
  ultimoTiempoGemini = Date.now();
}
...
async function generarNoticia(categoria, intento = 0) {
  ...
  try {
    // Esperar anti 429
    await esperarDelayGemini();

    // Rotar key
    slotActual = (slotActual + 1) % GEMINI_KEYS.length;
    const keyNum = slotActual + 1;
    const apiKey = GEMINI_KEYS[slotActual];

    console.log(`\n🚀 Key ${keyNum}/${GEMINI_KEYS.length} - ${categoria}`);

    const genAI = new GoogleGenerativeAI(apiKey);
    const model = genAI.getGenerativeModel({ model: 'gemini-2.5-flash' });

    const prompt = `Eres periodista de República Dominicana.
Escribe UNA noticia COMPLETAMENTE NUEVA sobre ${categoria}.
Debe ser diferente a noticias recientes.

Responde SOLO JSON (sin markdown):
{"titulo":"<80 chars","descripcion":"<160 chars","keywords":"palabra1,palabra2,palabra3","contenido":"<3 párrafos>"}`;
    ...
    let titulo = (data.titulo || '').replace(/[*_#`"]/g, '').trim().substring(0, 110);
    let desc = (data.descripcion || '').replace(/[*_#`]/g, '').trim().substring(0, 160);
    let keywords = (data.keywords || '').split(',').map(k => k.trim()).join(', ');
    let contenido = data.contenido || '';

    if (!titulo || titulo.length < 5) throw new Error('Título corto');
    if (!contenido || contenido.length < 50) throw new Error('Contenido corto');

    console.log(`✅ Generado: "${titulo.substring(0, 40)}..."`);

    // VERIFICAR DUPLICADO
    const dup = await verificarDuplicado(titulo, categoria);
    if (dup.esDuplicada) {
      console.log(`⚠️ DUPLICADA (${dup.similitud}%) - Reintentando...`);
      await pool.query(
    ...
    if (process.env.PEXELS_API_KEY) {
      try {
        const resp = await fetch(
          `https://api.pexels.com/v1/search?query=${encodeURIComponent(titulo.substring(0, 30))}&per_page=1`,
          { headers: { 'Authorization': process.env.PEXELS_API_KEY } }
        );
        const d = await resp.json();
        if (d.photos && d.photos[0]) imagen = d.photos[0].src.original;
      } catch (e) {
    ...
    // MONETIZACIÓN
    const tituloMone = monetizar(titulo, categoria);

    // PUBLICAR
    await pool.query(
      `INSERT INTO noticias (titulo, slug, seccion, contenido, imagen, descripcion_seo, keywords, redactor)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      [tituloMone, slugFinal, categoria, contenido, imagen, desc, keywords, `IA-${keyNum}`]
    );
    ...
    return { success: true, titulo: tituloMone, slug: slugFinal, key: keyNum };

  } catch (e) {
    console.error(`❌ Error:`, e.message);
    
    if (e.message && e.message.includes('429')) {
      console.log(`⚠️ Rate limit 429 - esperando 120s...`);
      ultimoTiempoGemini = Date.now() + 60000;
    }
...
app.get('/status', async (req, res) => {
  try {
    ...
    res.json({
      ...
      delay_segundos: DELAY_ENTRE_KEYS / 1000
    });
  } catch (e) {
...
app.post('/api/publicar', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, slug: slug_final });
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
});
...
app.get('/api/estadisticas', async (req, res) => {
  try {
    ...
    res.json({ totalNoticias: n.rows[0].count, totalVistas: v.rows[0].sum || 0 });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.post('/api/admin/config', auth, async (req, res) => {
  try {
    for (const k of ['enabled', 'instruccion_principal', 'enfasis', 'tono', 'extension', 'evitar']) {
      if (k in req.body) {
        await pool.query(
  ...
  try {
    const result = await pool.query(
      `SELECT gemini_key_num, titulo_intento, resultado FROM intentos_gemini
       ORDER BY timestamp_intento DESC LIMIT 100`
    );
...
app.get('/api/admin/comentarios', auth, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT c.id, c.nombre, c.texto, c.fecha, n.titulo FROM comentarios c
       LEFT JOIN noticias n ON c.noticia_id = n.id ORDER BY c.fecha DESC LIMIT 50`
    );
...
app.get('/api/coach', auth, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT seccion, COUNT(*) total, SUM(vistas) vistas, ROUND(AVG(vistas)) prom
    ...
    result.rows.forEach(r => {
      cats[r.seccion] = { total: r.total, vistas_promedio: r.prom || 0 };
    });
...
app.get('/api/telegram/status', auth, async (req, res) => {
  res.json({ token_activo: !!process.env.TELEGRAM_TOKEN, status: 'OK' });
});
...
schedule.scheduleJob('0 3 * * *', async () => {
  try {
    const result = await pool.query(
      `DELETE FROM noticias_publicadas_hoy WHERE fecha_publicacion < NOW() - INTERVAL '24 hours'`
    );
    console.log(`🧹 Limpieza: ${result.rowCount} registros`);
  } catch (e) {
    console.error('⚠️ Error limpieza');
  }
});
...
async function start() {
  try {
    ...
    app.listen(PORT, () => {
      ...
      console.log(`✅ Delay anti-429: ${DELAY_ENTRE_KEYS / 1000}s`);
