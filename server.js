 ...
 * Con manejadores de errores globales y health check
 */
...
process.on('uncaughtException', (err) => {
    ...
    // Logueamos pero no matamos el proceso inmediatamente
});
...
app.use(express.json({ limit: '50mb' })); // Reducido a 50mb
...
app.get('/noticias', async (req, res) => {
    try {
        ...
        const noticias = await Noticia.find().sort({ fecha: -1 }).limit(limit).skip(skip).lean();
...
app.get('/seccion/:nombre', async (req, res) => {
    try {
        ...
        const noticias = await Noticia.find({ seccion: nombre }).sort({ fecha: -1 }).limit(100).lean();
...
app.get('/buscar', async (req, res) => {
    try {
        ...
        res.json({ success: true, busqueda: q, total: noticias.length, noticias });
    } catch (error) {
...
app.get('/api/estadisticas', async (req, res) => {
    try {
        ...
        const noticiasHoy = await Noticia.countDocuments({ fecha: { $gte: hoy } });
...
app.post('/publicar', async (req, res) => {
    try {
        ...
        res.status(201).json({
            success: true,
            message: 'Publicado 🏮',
            noticia: { id: noticia._id, titulo: noticia.titulo, seccion: noticia.seccion, fecha: noticia.fecha }
        });
    } catch (error) {
...
app.post('/auth/registro', async (req, res) => {
    try {
        ...
        res.status(201).json({ success: true, message: 'Cuenta creada', token, usuario: { id: usuario._id, nombre: usuario.nombre, email: usuario.email } });
    } catch (error) {
...
app.post('/auth/login', async (req, res) => {
    try {
        ...
        const token = generarToken(usuario._id);
        res.json({ success: true, message: 'Sesión iniciada', token, usuario: { id: usuario._id, nombre: usuario.nombre, email: usuario.email } });
    } catch (error) {
...
app.post('/api/registrar-vista', async (req, res) => {
    try {
        ...
        await Noticia.findByIdAndUpdate(noticiaId, { $inc: { vistas: 1 } }, { new: true });
...
app.post('/api/configuracion', async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'Configuración guardada correctamente', config: configuracion.toObject() });
    } catch (error) {
