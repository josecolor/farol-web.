...
# --- CONFIGURACIÓN DE BASE DE DATOS (EL BOSQUE) ---
...
# Optimización de conexiones para evitar caídas en Railway
...
# --- DISEÑO CACHEADO PARA MÁXIMA VELOCIDAD ---
@lru_cache(maxsize=1)
def get_template():
    return '''
    <!DOCTYPE html>
    ...
    <head>
        ...
        <style>
            ...
            .com-item { font-size:0.8rem; margin-bottom:6px; border-bottom:1px solid #1a1a1a; }
            .com-item b { color:#ff8c00; }
            form { margin-top:10px; display:flex; gap:5px; }
            input { background:#1a1a1a; border:1px solid #333; color:#fff; padding:8px; border-radius:6px; flex:1; }
            button { background:#ff8c00; border:none; padding:8px 12px; border-radius:6px; font-weight:bold; }
        </style>
    </head>
    <body>
        <header>
        ...
        <div class="container">
            ...
            <div class="card">
                ...
                <div class="content">
                    ...
                    <h2>{{ n.titulo }}</h2>
                    <div style="color:#ccc; font-size:0.9rem;">{{ n.resumen|safe }}</div>
                </div>
                <div class="com-box">
                    ...
                    {% endfor %}
                    {% if user_id %}
                        <form action="/comentar/{{ n.id }}" method="post">
...
def register():
    if request.method == 'POST':
        ...
        # Lista de correos autorizados como Reporteros/Staff
        staff_emails = ["hsy@elfarol.com", "reportero2@elfarol.com", "reportero3@elfarol.com"]
        es_staff = True if email in staff_emails else False
    ...
    return '<body><form method="post"><h2>Registro</h2><input name="nombre" placeholder="Nombre" required><br><input name="email" type="email" placeholder="Email" required><br><input type="password" name="password" placeholder="Pass" required><br><button>Unirse</button></form></body>'
...
def login():
    if request.method == 'POST':
        ...
        if u and check_password_hash(u.password, request.form.get('password')):
            ...
            return redirect('/')
    return '<body><form method="post"><h2>Entrar</h2><input name="email" type="email" placeholder="Email" required><br><input type="password" name="password" placeholder="Pass" required><br><button>Entrar</button></form></body>'
...
def panel():
    if not session.get('es_staff'): return "No autorizado", 403
    if request.method == 'POST':
        ...
        db.session.commit()
        return redirect('/')
    return '<body><form method="post"><h2>Nueva Noticia</h2><input name="titulo" placeholder="Título" required><br><input name="location" placeholder="Ubicación" required><br><input name="imagen_url" placeholder="URL Foto"><br><textarea name="resumen" placeholder="Contenido" required></textarea><br><button>PUBLICAR</button></form></body>'
